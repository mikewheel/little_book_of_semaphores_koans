...
def test_solo_student_leaves_freely():
    log = EventLog()
    hall = make_hall(log)
    assert_completes(lambda: hall.student(1), timeout=5)
    assert log.count("dine:1") == 1
...
def test_newcomer_releases_waiter():
    ...
    try:
        ...
        runner.spawn(hall.student, 3, name="s3")  # newcomer sits down
...
def test_stress_everyone_leaves():
    n = 10
    log = EventLog()
