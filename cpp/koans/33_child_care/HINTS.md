...
walking off. One `std::counting_semaphore<>` *almost* carries the whole
design.
...
wrapping them in a `std::mutex` that only leavers touch. Then whoever holds
the mutex drains its full batch (blocking inside if permits are short), and
the next leaver queues behind it.
...
```text
permits    : counting_semaphore<>  starting at 0
...
adult_leave:  lock_guard g(leave_lock);
