...
std::shared_ptr<DiningHall> make_hall(std::shared_ptr<EventLog> log,
                                      std::shared_ptr<GateMap> gates) {
    ...
    return std::make_shared<DiningHall>(std::move(h));
}

// Fails fast (with the NotImplemented message) while the starter is empty:
// a lone student in an otherwise empty hall must dine and go unhindered.
void starter_tripwire() {
    auto log = std::make_shared<EventLog>();
    auto hall = make_hall(log, nullptr);
    assert_completes([hall] { hall->student(0); }, 5000ms,
                     "a solo student's visit");
}

}  // namespace

KOAN_TEST(solo_student_leaves_freely) {
    starter_tripwire();
    auto log = std::make_shared<EventLog>();
    auto hall = make_hall(log, nullptr);
    assert_completes([hall] { hall->student(1); }, 5000ms,
                     "a solo student's visit");
    KOAN_ASSERT_EQ(log->count("dine:1"), std::size_t{1});
    KOAN_ASSERT_EQ(log->count("leave:1"), std::size_t{1});
    log->assert_before("dine:1", "leave:1");
}
...
KOAN_TEST(newcomer_releases_waiter) {
    ...
    runner.spawn([hall] { hall->student(3); }, "s3");  // newcomer sits down
...
KOAN_TEST(pairs_leave_together) {
    ...
    runner.spawn([hall, gate2] { hall->student(2, [gate2] { gate2->wait_open(); }); },
                 "s2");
    log->wait_for_count("dine:1", 1, 5000ms);
...
KOAN_TEST(stress_everyone_leaves) {
    starter_tripwire();
    constexpr int n = 10;
    auto log = std::make_shared<EventLog>();
    auto gates = std::make_shared<GateMap>();
