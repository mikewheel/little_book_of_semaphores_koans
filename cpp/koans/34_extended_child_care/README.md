...
Edit `extended_child_care.hpp`. Implement `ExtendedChildCare` (constructed
with `ratio`) with the same four methods as koan 33:
...
- With `std::condition_variable` this koan is almost anticlimactic: a
  scoreboard under a `std::mutex`, `child_enter` does
  `cv.wait(lock, [&]{ return children < ratio * adults; })`, `adult_leave`
  waits on `children <= ratio * (adults - 1)`, and every mutation ends in
  `notify_all()`. The predicate re-check on wake — the thing a bare
  semaphore cannot do — is built into the primitive, and spurious wakeups
  are harmless for free.
- The semaphore version is the pedagogical point: without predicate
  re-checks, the *signaling* side must complete the waiter's state
  transition before waking it ("I'll do it for you"). Once you've written
  both, you'll recognize which discipline your favorite cv idiom is
  quietly automating.
- Efficiency note for the cv version: `notify_all` on every change is
  simple and correct; targeted `notify_one` is a classic source of lost
  wakeups when two different predicates share one cv. Two cvs (one per
  waiting population) is the tidy middle ground.
- UB pitfall: keep every read of the scoreboard under the mutex —
  "harmless" unlocked reads of `children`/`adults` are data races, and the
  optimizer is allowed to make them lie to you.
